...
let continuebut = {
  ...
  callback: async (client, interaction) => {
    try {
      let passed = interaction.customId.split("|")[2]
      ...
      if (verificationType === 0 || verificationType === undefined) {
        const customid = passed ? `Verification|${interaction.customId.split("|")[1]}|${passed}` : `Verification`
        const userModalConfig = await getModal(client, "username");
        const emailModalConfig = await getModal(client, "email");
        return interaction.showModal(
          modalBuilder(`${customid}`, userModalConfig.title, [
            {
