module.exports = {
  ...
  callback: (client, interaction) => {
    let inputColor = interaction.fields.getTextInputValue('color').replace(/\s+/g, '').trim();
    let convertedColor;
    
    try {

      convertedColor = processColor(inputColor);
    
      
      if (convertedColor === null) {
        return interaction.reply({
          content: "Invalid color format. Try: Hex (#RRGGBB) or named colors (purple)",
          ephemeral: true
        });
      }
      

      interaction.message.embeds[0].data.color = convertedColor;
      interaction.update({
      ...
      interaction.reply({
        content: "An error occurred while processing the color. Please try again with a different format.",
        ephemeral: true
      });
    }
  }
...
function processColor(inputColor) {
  ...
  const namedColors = {
    ...
    "gray": "#808080", "grey": "#808080", "lime": "#00ff00", "maroon": "#800000",
    "navy": "#000080", "olive": "#808000", "teal": "#008080", "violet": "#ee82ee",
    "indigo": "#4b0082", "turquoise": "#40e0d0", "silver": "#c0c0c0", "gold": "#ffd700",
  ...
  if (namedColors[inputColor.toLowerCase()]) {
    inputColor = namedColors[inputColor.toLowerCase()];
  }
  

  if (/^#?([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6}|[0-9A-Fa-f]{8})$/.test(inputColor)) {
